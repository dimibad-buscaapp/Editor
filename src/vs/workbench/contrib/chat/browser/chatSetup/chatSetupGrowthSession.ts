...
export class GrowthSessionController extends Disposable implements IChatSessionItemController {
	...
	get items(): readonly IChatSessionItem[] {
		...
		return [{
			...
			label: localize('growthSession.label', "Try Princy Ai"),
			description: localize('growthSession.description', "Princy Ai is available with Llama-backed workspace context."),
...
export class GrowthSessionOpenerParticipant implements ISessionOpenerParticipant {
	...
	async handleOpenSession(accessor: ServicesAccessor, session: IAgentSession, _openOptions?: ISessionOpenOptions): Promise<boolean> {
		...
		const opts: IChatViewOpenOptions = {
			...
			isPartialQuery: true,
			previousRequests: [{
				request: localize('growthSession.previousRequest', "Tell me about Princy Ai!"),
				// allow-any-unicode-next-line
				response: localize('growthSession.previousResponse', "Welcome to Princy Ai, your Llama-backed coding assistant. Use the Princy Ai panel for Composer, Shadow Context, Code Graph, terminal fixes, and multi-model agents."),
			}],
		};
