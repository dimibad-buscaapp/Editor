# Deploy Fase 10 - Princy Automations
# Admin no VPS: powershell -ExecutionPolicy Bypass -File deploy\windows\deploy-fase10-automacoes.ps1
...
Write-Host ""
Write-Host "=== Deploy Fase 10 - Princy Automations ===" -ForegroundColor Cyan
...
$checks = @(
	...
	@{ Label = "Automations /api/automations"; Url = "http://127.0.0.1:3210/api/automations" },
	@{ Label = "Automations publico /princy-api"; Url = "https://princyai.com/princy-api/api/automations" }
)
$allOk = $true
foreach ($c in $checks) {
	try {
		...
		$ok = $null -ne $r
		if ($c.Url -match '/api/automations' -and $r.ok -ne $true) { $ok = $false }
...
Write-Host ""
if ($allOk) {
	Write-Host "Fase 10 deploy concluido." -ForegroundColor Green
	Write-Host "No chat Princy AI: modo Automations -> Creator -> Gerar / Agendar / Executar / Pipeline"
} else {
